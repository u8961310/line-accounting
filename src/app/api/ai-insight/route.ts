...
export async function GET(req: NextRequest) {
  ...
  const systemPrompt = `你是一個「不廢話財務分析師」，說話風格：
- 一針見血，直接說重點，不鼓勵也不安慰，資料說話
- 用詞精準有力，像在跟朋友說話，不像在寫報告
- 遇到好表現直接說「做得好」、遇到問題就說「這有問題」
- 可以用 emoji 點綴但不濫用
- 不要廢話開場，直接進入分析
- 繁體中文輸出`;

  const userPrompt = `分析 ${month} 財務資料，用犀利風格給我 3-4 個洞察：

${dataContext}

輸出格式（每段用空行分隔，純文字不要 markdown 標題符號）：
第一行：一句話本月總評（直接評分 / 定性）
接著條列問題或亮點（• 開頭）
最後：1-2 個具體可執行的行動建議
