...
export default function CsvImport({ lineUserId, onImportComplete }: CsvImportProps) {
  ...
  // 需要顯示信用卡選擇的 PDF：只有未指定銀行（auto）或永豐信用卡
  const pdfNeedsCcSelection = selectedBank === "auto" || selectedBank === "sinopac_cc";

  function handleFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    const file = e.target.files?.[0];
    if (!file) return;
    if (file.name.toLowerCase().endsWith(".pdf") && pdfNeedsCcSelection) {
    ...
    <div className="rounded-2xl border" style={{ background: "var(--bg-card)", borderColor: "var(--border)", boxShadow: "var(--card-shadow)" }}>
      ...
      <div className="px-6 py-5 flex items-center gap-3" style={{ borderBottom: "1px solid var(--border-inner)" }}>
        ...
        <div>
          ...
          <p className="text-[14px]" style={{ color: "var(--text-sub)" }}>支援 CSV · XLS · XLSX · PDF（永豐信用卡）</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 divide-y md:divide-y-0 md:divide-x" style={{ ["--tw-divide-color" as string]: "var(--border-inner)" }}>
        ...
        <div className="p-6 space-y-4">
          ...
          {!isPdfSelected && (
            <div>
              <label className="text-[14px] font-semibold mb-1.5 block" style={{ color: "var(--text-sub)" }}>銀行 / 發卡行</label>
          ...
          {isPdfSelected && pendingPdfFile && (
            <div className="rounded-xl p-4 space-y-3" style={{ background: "var(--bg-input)", border: "1px solid var(--border)" }}>
              <div className="flex items-center gap-2">
                <span className="text-[14px] font-bold px-2 py-0.5 rounded-full" style={{ background: "rgba(124,58,237,0.15)", color: "#A78BFA" }}>PDF</span>
                <p className="text-[14px] truncate" style={{ color: "var(--text-primary)" }}>{pendingPdfFile.name}</p>
              </div>
              <div>
                <label className="text-[14px] font-semibold mb-1.5 block" style={{ color: "var(--text-sub)" }}>關聯信用卡（選填）</label>
              ...
              <div className="flex gap-2">
                ...
                <button onClick={() => uploadFile(pendingPdfFile)} disabled={isUploading}
                  className="flex-1 py-2 rounded-xl text-[14px] font-bold text-white disabled:opacity-50"
          ...
          >
            {isUploading ? (
              <div className="flex flex-col items-center gap-2">
                ...
                <p className="text-[14px]" style={{ color: "var(--text-sub)" }}>正在匯入...</p>
              </div>
            ) : (
              <div className="flex flex-col items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-[14px]" style={{ color: "var(--text-sub)" }}>CSV · XLS · XLSX · PDF</p>
                </div>
              </div>
            )}
          </div>
        ...
        <div className="p-6">
          <p className="text-[14px] font-bold tracking-widest uppercase mb-4" style={{ color: "var(--accent)" }}>支援格式</p>
          <div className="space-y-1">
            {FORMAT_LIST.map((f, i) => (
              <div key={i} className="flex items-center gap-3 py-2 px-3 rounded-xl transition-colors"
                ...
                <span className="text-[14px] font-bold px-1.5 py-0.5 rounded flex-shrink-0"
                  style={f.type === "存款"
                    ? { background: "rgba(59,130,246,0.12)", color: "var(--accent)" }
                    : { background: "rgba(192,132,252,0.12)", color: "#A78BFA" }}>
                  {f.type}
                </span>
                <span className="text-[14px] font-medium flex-1" style={{ color: "var(--text-primary)" }}>{f.bank}</span>
                <span className="text-[14px] font-mono px-2 py-0.5 rounded" style={{ background: "var(--bg-input)", color: "var(--text-sub)" }}>{f.fmt}</span>
              </div>
            ))}
          </div>
          <div className="mt-4 pt-4 flex items-start gap-2" style={{ borderTop: "1px solid var(--border-inner)" }}>
            <span className="text-[14px] mt-0.5">💡</span>
            <p className="text-[14px]" style={{ color: "var(--text-sub)" }}>重複資料自動略過（相同日期 + 金額 + 來源）</p>
          </div>
        </div>
      </div>
      ...
      {result && (
        <div className="px-6 pb-6">
          ...
          {result.success ? (
                <>
                  <p className="text-[14px] font-semibold" style={{ color: "#10B981" }}>{result.message}</p>
                  <div className="mt-1.5 flex gap-4 text-[14px]" style={{ color: "#059669" }}>
                    <span>匯入 <b>{result.imported}</b> 筆</span>
                    <span>略過 <b>{result.skipped}</b> 筆</span>
                    {result.source !== "unknown" && <span>來源：{result.source}</span>}
                  </div>
                  {result.bill && (
                    <p className="mt-1 text-[14px]" style={{ color: "#059669" }}>
                  ...
                  {result.errors.length > 0 && (
                    <ul className="mt-1.5 text-[14px] list-disc list-inside" style={{ color: "#F59E0B" }}>
                      {result.errors.slice(0, 3).map((err, i) => <li key={i}>{err}</li>)}
                    </ul>
                  )}
                  {result.transactions && result.transactions.length > 0 && (
                    <div className="mt-3 rounded-xl overflow-hidden" style={{ border: "1px solid rgba(16,185,129,0.25)" }}>
                      <div className="px-3 py-2 text-[14px] font-bold tracking-wide" style={{ background: "rgba(16,185,129,0.12)", color: "#10B981" }}>
                        已匯入明細
                      </div>
                      <div className="overflow-y-auto" style={{ maxHeight: 220 }}>
                        {result.transactions.map((tx, i) => (
                          <div key={i}
                            className="flex items-center gap-2 px-3 py-2 text-[14px]"
                            style={{ borderTop: i > 0 ? "1px solid rgba(16,185,129,0.12)" : undefined }}>
                            <span className="flex-shrink-0 w-20 text-[14px]" style={{ color: "var(--text-muted)" }}>{tx.date}</span>
                            <span className="flex-shrink-0 px-1.5 py-0.5 rounded text-[14px] font-bold"
                              style={tx.type === "收入"
                            ...
                            <span className="flex-shrink-0 font-bold"
                              style={{ color: tx.type === "收入" ? "#10B981" : "#F87171" }}>
                              {tx.type === "收入" ? "+" : "−"}NT$ {tx.amount.toLocaleString()}
                            </span>
                          </div>
                        ))}
                      </div>
                ...
                <>
                  <p className="text-[14px] font-semibold" style={{ color: "#EF4444" }}>{result.error ?? "匯入失敗"}</p>
                  {result.errors?.length > 0 && (
                    <ul className="mt-1 text-[14px] list-disc list-inside" style={{ color: "#F87171" }}>
                      {result.errors.slice(0, 3).map((err, i) => <li key={i}>{err}</li>)}
                    </ul>
                  )}
                </>
