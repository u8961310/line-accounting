...
## 財務目標（背景資訊，供 AI 分析參考）
- 備用金目標：$171,000（3 個月支出）
- 優先還清：凱基速還金（利率 16%，剩 $46,239）
...
- [ ] Claude 批次「其他」分類清理：查出所有 category=「其他」交易，Claude 分析 note 批次建議分類，Dashboard 工具區顯示建議列表一鍵套用

### 資料分析強化（🟡 中）
- [x] 收支年同期比較（YoY）：在月份對比卡或進階分析新增「今年 N 月 vs 去年 N 月」，追蹤長期財務進步
- ~~帳單行事曆 .ics 匯出~~ → 改用 LINE 推播提醒（帳單到期前 3 天自動推送）
- ~~財務日記（月度一句話）~~ → 不需要
- [x] 每日財務箴言：圖表 Tab 頂端顯示聖經理財金句 + 關聖帝君聖訓，每天自動換一則，可手動 🔀 換一則
- [ ] 支出心情分佈圖：mood 欄位（衝動/計畫/必要）月趨勢 Pie + Bar 圖，顯示衝動消費佔比與金額，圖表 Tab 新增卡片
- [ ] 省錢月挑戰：每月自訂分類支出上限挑戰，首頁顯示進度條 + 每日剩餘可花額度，localStorage 存設定
- [ ] 貸款提前還款模擬器：負債管理頁加「每月多還 X 元」滑桿，即時試算提前還清月數與節省利息（對凱基 16% 特別有用）
...
- [ ] **財務健康評分快照**：移除手動「📌 記錄」按鈕，改為每次載入圖表 Tab 時自動 upsert 當月快照（趨勢圖保留）；移除 `/api/health-score/snapshots` 的手動呼叫邏輯
- [ ] **使用說明 Tab**：把「使用說明」從 Tools 下拉移除，改為 Header 右側的 `?` 按鈕展開 Modal，減少 Tab 數量
- [ ] **淨資產快照手動記錄**：與健康評分快照相同，移除「記錄快照」按鈕，改為自動 upsert（目前兩套快照邏輯重複）
