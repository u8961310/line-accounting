...
- [ ] 分類預算快覽列：圖表 Tab 頂端顯示所有分類的 已用/預算 進度條，超標變紅（目前需切到預算 Tab）
- [ ] 每日消費熱力圖：7×N 格 heatmap，X=星期 Y=週次，顏色深淺代表當日消費金額
- [ ] 消費月曆升級：日曆格子顯示當日支出總額（顏色編碼），hover 展開明細
- [ ] 搜尋歷史記錄：快速搜尋 Bar 顯示最近 5 筆紀錄（localStorage），點一下套用
- [ ] 淺色主題：新增一個 Light 主題選項（目前三個主題均為深色）
...
#### 交易記錄頁
- [ ] 篩選快速日期按鈕：今天 / 本週 / 本月 / 上月 一鍵套用
- [x] 無限滾動：滾到底部自動載入下一頁，取代分頁按鈕
- [ ] 空狀態改善：搜尋無結果時顯示「清除篩選」按鈕
- [ ] 行內金額編輯：點擊金額直接 inline 編輯，不用開 modal
